...
//  Пользователь - СправочникСсылка.ФизическиеЛица - Пользователь, который смотрит товар
//  Дата - Дата - Дата на которую проверяется возможность покупки, по умолчанию - текущая дата
//   
// Возвращаемое значение:
//  Структура
...
//
Функция ОписаниеМерча(Ссылка, Пользователь, Дата) Экспорт
	ОбщийМодульСервер.ДатаЗапросаПоУмолчанию(Дата);
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(БалансПользователяОстатки.ЗолотоОстаток, 0) >= НоменклатураМерча.СтоимостьЗолото
	|		И ЕСТЬNULL(БалансПользователяОстатки.СереброОстаток, 0) >= НоменклатураМерча.СтоимостьСеребро
	|			Тогда ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ВозможноКупить
	|ИЗ
	|	Справочник.НоменклатураМерча КАК НоменклатураМерча
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.БалансПользователя.Остатки(&Дата, ФизическоеЛицо = &Пользователь) КАК
	...
	Если НЕ СтруктураВозврата.Картинка = Неопределено Тогда
		СтруктураВозврата.Картинка = ПоместитьВоВременноеХранилище(Выборка.Картинка.Получить());
	КонецЕсли;
	Если Выборка.ВозможноКупить Тогда

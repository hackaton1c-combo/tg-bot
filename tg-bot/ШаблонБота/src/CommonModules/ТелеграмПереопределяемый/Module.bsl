...
Процедура ОбработатьСообщениеЧата(ТекстСообщения, 
	...
	Если ТекстСообщения = "Каталог бонусов" Тогда		
		ДополнительныеПараметры = Телеграм.НовыеДополнительныеПараметрыСообщенияФизЛицу();
		ДополнительныеПараметры.ОтобразитьКаталог = Перечисления.ВидыКаталогов.Награды;
...
Процедура ПриПолученииЭлементовКаталога(ЭлементыКаталога, ФизическоеЛицо, Каталог, СтрокаПоиска) Экспорт
	...
	Если Каталог = Перечисления.ВидыКаталогов.Награды Тогда
		ЭлементыКаталога = Справочники.НоменклатураМерча.ПолучитьСписокМерча();
	КонецЕсли;
...
Процедура ПриПолученииСсылкиНаОбъект(Каталог, Идентификатор, Ссылка) Экспорт
	...
	Если Каталог = Перечисления.ВидыКаталогов.Награды Тогда			
		Ссылка = Справочники.НоменклатураМерча.ПолучитьСсылку(УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ПриПолученииОписанияОбъекта(Ссылка, ОписаниеОбъекта) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.НоменклатураМерча") Тогда
		ОписаниеОбъекта = Справочники.НоменклатураМерча.ОписаниеМерча(Ссылка);
	КонецЕсли;
...
Процедура ПриОбработкеКомандыОбъекта(ФизЛицо, Ссылка, ИдентификаторКоманды, ТекстСообщения, Команды) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.НоменклатураМерча") Тогда
		Результат = Справочники.НоменклатураМерча.КупитьМерч(ФизЛицо, Ссылка);		
		ТекстСообщения = Результат.Сообщение;
		Команды = Результат.Команды;
	КонецЕсли;

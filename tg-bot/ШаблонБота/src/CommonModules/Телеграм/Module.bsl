...
Функция ОбработатьКомандуКаталога(КонтекстКоманды)
	...
	Иначе
		...
		Если Не ОписаниеОбъекта.БыстрыйОтвет Тогда
			Ответ.Вставить("reply_markup", 
				КоманднаяПанельОбъекта(КонтекстКоманды.КомандаСообщения, ОписаниеОбъекта.Команды));
		Иначе
			БыстрыйОтвет = Новый Структура;
			БыстрыйОтвет.Вставить("force_reply", Истина);
			Ответ.Вставить("reply_markup", БыстрыйОтвет);
		КонецЕсли;

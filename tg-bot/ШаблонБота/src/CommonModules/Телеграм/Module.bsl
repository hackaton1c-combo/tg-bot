...
Функция ОбработатьКомандуСообщения(ДанныеЗапроса)
	...
	Если ЭтоСообщениеКаталог(ТекстСообщения) Тогда
		Возврат ОбработатьКомандуКаталога(КомандаСообщения, ТекстСообщения, ИдентификаторСообщения, ИдентификаторЧата);
	КонецЕсли;
	
	Если ЭтоСообщениеОбъект(ТекстСообщения) Тогда
		Возврат ОбработатьКомандуОбъекта(КомандаСообщения, ТекстСообщения, ИдентификаторСообщения, ИдентификаторЧата);
	КонецЕсли;	
...
Функция ОбработатьКомандуКаталога(КомандаСообщения, ТекстСообщения, ИдентификаторСообщения, ИдентификаторЧата)
	
	СтрокиТекстаСообщения = СтрРазделить(ТекстСообщения, Символы.ПС);
	...
	Если СтрНачинаетсяС(КомандаСообщения, "page_") Тогда
		
		НомерСтраницы = Число(СтрЗаменить(КомандаСообщения, "page_", ""));
			
		ТекстНовогоСообщения = "";
		
		КомандаУправленияКлавиатурой = КоманднаяПанельКаталога(Каталог,
			ТекстНовогоСообщения, НомерСтраницы, СтрокаПоиска);	
		
		Ответ = Новый Структура;
		Ответ.Вставить("method", "editMessageText");
		Ответ.Вставить("chat_id", ИдентификаторЧата);
		Ответ.Вставить("message_id", ИдентификаторСообщения);
	...
	Иначе
		...
		ТелеграмПереопределяемый.ПриПолученииСсылкиНаОбъект(Каталог, КомандаСообщения, Ссылка);
		...
		Ответ.Вставить("reply_markup", КоманднаяПанельОбъекта(КомандаСообщения, ОписаниеОбъекта.Команды));
...
Функция ОбработатьКомандуОбъекта(КомандаСообщения, ТекстСообщения, ИдентификаторСообщения, ИдентификаторЧата)
	...
	ЧастиКоманды = СтрРазделить(КомандаСообщения, "/");
	...
	Если Не ПустаяСтрока(ТекстКомментария) Тогда
		ОтправитьТекстовоеСообщение(ТекстКомментария, ИдентификаторЧата);
	КонецЕсли;
...
Функция КоманднаяПанельКаталога(Каталог, ТекстСообщения, Страница = 0, СтрокаПоиска = "")
	
	ЭлементыКаталога = Новый Массив;
	ТелеграмПереопределяемый.ПриПолученииЭлементовКаталога(ЭлементыКаталога, Каталог, СтрокаПоиска);	

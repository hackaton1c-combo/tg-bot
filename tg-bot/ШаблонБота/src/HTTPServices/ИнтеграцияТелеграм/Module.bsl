...
Процедура ЗаписатьНеобработанноеСообщение(Запрос)
	...
	Попытка
		ОбъектJSON = HTTPСервисы.ПрочитатьИзJSON(Запрос.ПолучитьТелоКакСтроку());
		ТелоЗапроса = HTTPСервисы.ЗначениеВJSON(ОбъектJSON, Символы.Таб);
	Исключение
		ТелоЗапроса = Запрос.ПолучитьТелоКакСтроку();
	КонецПопытки;
	
	ЛогЗапроса.Добавить(ТелоЗапроса);
